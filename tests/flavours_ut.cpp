...
TEST(Separators, EqOrSpaceSucces) {
    ...
    TestConfig cfg;
    ostringstream err;
    auto ok = cfg.Parse({ "-f", "-1", "-b=1", "-w", "WAT IS", "--wat=WAT" }, err);
    //~ cerr << err.str() << endl;
    ASSERT_EQ(true, ok);
...
TEST(Separators, EqSeparatorExtraVal) {
    Config cfg;
    NamedArg<string> foo(cfg, "foo", "FOO");
    FreeArg<string, false> bar(cfg, "bar", "BAR");

    ostringstream err;
    auto ok = cfg.Parse({ "--foo=FOO", "BAR" }, err);
    //~ cerr << err.str() << endl;
    ASSERT_EQ(true, ok);
